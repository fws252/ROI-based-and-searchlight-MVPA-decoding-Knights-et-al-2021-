...
nRuns=length(find
(order(subNo,:))); %nRuns=length(order); %nRuns=6 view, pantomime

% the path to the files
dir_name=sprintf('Z:\\fMRI_bial\\%s\\%s_pp\\%s_OtherVTC\\%s_MVPAVTC\\',snames{subNo},snames{subNo},snames{subNo},snames{subNo});
loc_name=sprintf('%s_vois.voi',snames{subNo});
...
for r=1:nRuns
    if strcmp(snames{subNo},'CR09') || strcmp(snames{subNo},'DT12') || strcmp(snames{subNo},'LP12') || strcmp(snames{subNo},'QP18') || strcmp(snames{subNo},'RL17') || strcmp(snames{subNo},'RS06') || strcmp(snames{subNo},'ST28') || strcmp(snames{subNo},'TS12')
    ...
    prtNames{r}=['Z:\fMRI_bial\PRT\new prts\' sprintf('toolstudy1_%s_Run%d_Order%d.prt',snames{subNo},r,order(subNo,r))];
